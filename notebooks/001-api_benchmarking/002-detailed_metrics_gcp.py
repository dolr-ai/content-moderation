...
results_df["predicted_category"] = results_df["processed_response"].apply(
    ...
    )  # default to clean if confidence is below threshold
)
...
def calculate_evaluation_metrics(results_df):
    ...
    error_rate = len(
        results_df[results_df["raw_api_response"].apply(lambda x: "error" in x)]
    ) / len(results_df)
    ...
    for threshold in thresholds:
        ...
        predictions = results_df[score_columns].apply(
            lambda row: (
                ...
                else list(PRIMARY_CATEGORY_MAP.keys())[row.argmax()]
            ),
            axis=1,
        )
        ...
        precision, recall, f1, _ = precision_recall_fscore_support(
            results_df["actual_category"], predictions, average="macro"
        )
...
def plot_category_score_distributions(results_df, categories):
    ...
    for actual_category in categories:
        ...
        correct_scores = (
            category_data[correct_mask]["processed_response"]
            .apply(
                lambda x: (
                    x["scores"][actual_category]
                    if isinstance(x, dict)
                    and x["scores"][actual_category] >= CONFIDENCE_THRESHOLD
                    else None
                )
            )
            .dropna()
        ...
        for pred_category in categories:
            if pred_category != actual_category:
                ...
                misclass_scores = (
                    category_data[misclass_mask]["processed_response"]
                    .apply(
                        lambda x: (
                            x["scores"][pred_category]
                            if isinstance(x, dict)
                            and x["scores"][pred_category] >= CONFIDENCE_THRESHOLD
                            else None
                        )
                    )
                    .dropna()
        ...
        if scores_data:
            ...
            plt.title(
                f"Score Distribution for Actual Category: {actual_category}\n(Showing scores of predicted categories)",
                pad=20,
            )
            plt.ylabel("Confidence Score")
            plt.grid(True, axis="y", linestyle="--", alpha=0.7)
            plt.xticks(
                range(1, len(plot_labels) + 1), plot_labels, rotation=45, ha="right"
            )
            plt.tight_layout()
            plt.show()
...
def print_high_confidence_misclassifications(
    df, category, is_false_negative=True, n_samples=100
):
    """
...
def analyze_all_categories(results_df, primary_categories, project_root, n_samples=100):
    ...
    for category in [cat for cat in primary_categories.keys() if cat != "clean"]:
        ...
        output.append(
            print_high_confidence_misclassifications(
...
def print_correct_classifications(df, n_samples=5):
    """
    Print random examples where the model correctly classified each category.

    Args:
        df (pd.DataFrame): DataFrame containing the classification results
        n_samples (int): Number of samples to display for each category
    """
    ...
    for category in PRIMARY_CATEGORY_MAP.keys():
        ...
        correct_mask = (df["actual_category"] == category) & (
            df["predicted_category"] == category
        )
        correct_samples = df[correct_mask].sample(
            min(n_samples, sum(correct_mask)), random_state=4567
        )

        for idx, row in correct_samples.iterrows():
            confidence_score = row["processed_response"]["scores"][category]
            print(f"\n**Text:** ```{row['text'][:1000]} ...```")
...
# Run the function
print_correct_classifications(results_df)
