...
def calculate_dual_prediction_metrics(
    ...
    for category in valid_categories:
        ...
        predicted_positive = (df['new_primary'] == category) | (df['new_secondary'] == category)
...
def create_precision_recall_matrix(
    ...
    if use_dual_prediction:
        df['combined_pred'] = df.apply(
            lambda x: x['new_primary'] if x['actual_category'] == x['new_primary']
            else x['new_secondary'] if x['actual_category'] == x['new_secondary']
            else x['new_primary'],
            axis=1
        )
...
"""
    for category in CATEGORY_ORDER:
        ...
        misclassified = df[
            ...
            ((df['new_secondary'] != category) | (df['new_secondary'].isin(['none', 'None'])))
        ]
...
def main(file_path: str):
    ...
    print(f"Old Model Accuracy: {accuracy_metrics['old_model_accuracy']:.3f} ({accuracy_metrics['old_correct']}/{accuracy_metrics['total_samples']} samples)")
    print(f"New Model Accuracy (Primary + Secondary): {accuracy_metrics['new_model_accuracy']:.3f} ({accuracy_metrics['new_correct']}/{accuracy_metrics['total_samples']} samples)")
    print(f"New Model Accuracy (Primary Only): {accuracy_metrics['primary_only_accuracy']:.3f} ({accuracy_metrics['primary_correct']}/{accuracy_metrics['total_samples']} samples)")
